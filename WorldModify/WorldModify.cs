            ...
            switch (args.Parameters[0].ToLowerInvariant())
            {
                ...
                case "mode":
                    if(args.Parameters.Count<string>()==1){
                        op.SendInfoMessage("世界难度: {0}", _worldModes.Keys.ElementAt(Main.GameMode));
                        op.SendInfoMessage("用法：/wm mode <难度>");
                        op.SendInfoMessage("可用的难度：{0}", String.Join(", ", _worldModes.Keys));
                        return;
                    }
                    ...
                    if (int.TryParse(args.Parameters[1], out int mode))
                    {
                        if (mode < 1 || mode > 4)
                        {
                            ...
                            op.SendErrorMessage("可用的难度：{0}", String.Join(", ", _worldModes.Keys));
                            return;
                        }
                    }
